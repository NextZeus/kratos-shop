...
import (
	...
	"github.com/nextzeus/kratos-shop/app/user/service/internal/pkg/util"
	"time"
)
...
func (r *userRepo) GetUser(ctx context.Context, id int64) (*biz.User, error) {
	...
	if err != nil {
		...
		r.setUserCache(ctx, cacheKey, target)
	}
	return &biz.User{
		Id:       target.ID,
...
func (r *userRepo) FindByUsername(ctx context.Context, username string) (*biz.User, error) {
	...
	target, err := r.getUserFromCache(ctx, cacheKey)
	if err != nil {
		target, err = r.data.db.User.Query().Where(user.UsernameEQ(username)).Only(ctx)
		if err != nil {
			return nil, biz.ErrUserNotFound
		}
	}
...
func (r *userRepo) setUserCache(ctx context.Context, key string, user *ent.User) {

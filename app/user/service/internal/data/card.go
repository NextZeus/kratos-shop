...
func (r *cardRepo) CreateCard(ctx context.Context, c *biz.Card) (*biz.Card, error) {
	po, err := r.data.db.Card.Create().SetCardNo(c.CardNo).SetCcv(c.CCV).SetExpires(c.Expires).Save(ctx)
	if err != nil {
		return nil, err
	}
	return &biz.Card{
...
func (r *cardRepo) ListCard(ctx context.Context, uid int64) ([]*biz.Card, error) {
	pos, err := r.data.db.User.Query().Where(user.ID(uid)).QueryCards().All(ctx)
	if err != nil {
		return nil, err
	}
	rv := make([]*biz.Card, 0)
